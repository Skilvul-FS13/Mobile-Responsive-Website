...
Promise.all([userPostData, userData])
  ...
  .then((postByUsername) => {
    ...
    const getPostFiltered = getPostByUsername.map((filter) => showUserPostsByUsername(filter)).join('');
...
const showUserProfileByUsername = (u) => {
  ...
  <div class="container-lg d-flex flex-column profile-banner">
    <img class="object-fit-cover rounded-bottom-2" src="${u.banner}" alt="" height="300" loading="lazy" />
  </div>
