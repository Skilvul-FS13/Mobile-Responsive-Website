...
Promise.all([userPostData, userData])
  .then(([posts, users]) => {
    const getPostByUser = posts.map((post) => {
      ...
      // console.log('🚀 ~ file: community.js:24 ~ getPostByUser ~ user:', user);
      return { ...post, avatar, username, firstName, lastName };
    });
    console.log('🚀 ~ file: community.js:28 ~ .then ~ getPostByUser:', getPostByUser);
    return getPostByUser;
  })
  .then((postById) => {
    // console.log('🚀 ~ file: community.js:31 ~ postById:', postById);
