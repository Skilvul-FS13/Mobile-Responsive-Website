...
Promise.all([userPostData, userData])
  ...
  .then((profileDetail) => {
    ...
    const getProfileDetail = profileDetail.filter((postByUser) => {
      return postByUser.username == profile;
    });

    const getProfileFiltered = getProfileDetail.map((filter) => showUserPostsByUsername(filter)).join('');

    if (getProfileFiltered) {
      postContainer.innerHTML = getProfileFiltered;
    } else {
      postContainer.innerHTML = `<div>Pengguna tidak ditemukan</div>`;
    }
  })
